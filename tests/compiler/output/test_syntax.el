...
(puthash 'test_syntax (let*
  (
  (test-cases nil)
  (x nil)
  (y nil))
  (setq test-cases (lambda (_a1) (if
    (funcall (lambda (x) (funcall (gethash 'eq-int (gethash 'ff (gethash 'std oc))) x 1)) _a1)
    (funcall (lambda (x) 2) _a1)
    (if
      (funcall (lambda (x) (funcall (gethash 'eq-int (gethash 'ff (gethash 'std oc))) x 2)) _a1)
      (funcall (lambda (x) 3) _a1)
      (if
        (funcall (lambda (x) (funcall (gethash 'eq-int (gethash 'ff (gethash 'std oc))) x 3)) _a1)
        (funcall (lambda (x) 4) _a1)
        (error "cases error"))))))
  (setq x (oc-hash-from-alist (list
    'foo 1)))
  (setq y (let*
    (
    (_obj nil)
    (_clone nil))
    (setq _obj x)
    (setq _clone (funcall (gethash 'clone-object oc) _obj))
    (puthash 'foo 2 _clone)
    _clone))
  (oc-hash-from-alist (list
    'test-cases test-cases
    'item1 (funcall (gethash 'check-eq (gethash 'testing oc)) (gethash 'int (gethash '_eq (gethash 'instances oc))) (funcall test-cases 2) 3)
    'x x
    'y y
    'item2 (funcall (gethash 'check-eq (gethash 'testing oc)) (gethash 'int (gethash '_eq (gethash 'instances oc))) (gethash 'foo y) 2)))) oc)
